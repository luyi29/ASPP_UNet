...
class ConfusionMatrix(object):
    ...
    def compute(self):
        #每一行代表真实类别被预测成其他类的像素数
        h = self.mat.float()
        for i in range(h.shape[0]):
            if h[i,:].sum()==0 and h[:,i].sum()==0:
                h[i,i] = 1
        # 计算全局预测准确率(混淆矩阵的对角线为预测正确的个数) TP/sum 
        acc_global = torch.diag(h).sum() / h.sum()
        # 计算每个类别的准确率  TP/TP+TN
        acc = torch.diag(h) / h.sum(1)
        # 计算每个类别预测与真实目标的iou
        iu = torch.diag(h) / (h.sum(1) + h.sum(0) - torch.diag(h))
        dice_ = 2*torch.diag(h) / (h.sum(1) + h.sum(0))
        return acc_global, acc, iu,dice_
    ...
    def __str__(self):
        acc_global, acc, iu, dice_= self.compute()
        return (
            'global correct: {:.1f}\n'
            'average row correct: {}\n'
            'IoU:                 {}\n'
            ...
            'dice_:               {}').format(
                ...
                iu.mean().item() * 100,
                ['{:.1f}'.format(i) for i in (dice_ * 100).tolist()])
